...
在 GitHub Actions / Cloudflare 生产变量中添加或更新：

- GitHub repository variables：`NEXT_PUBLIC_UMAMI_WEBSITE_ID`、`NEXT_PUBLIC_UMAMI_SCRIPT_URL`、`NEXT_PUBLIC_UMAMI_ALLOWED_DOMAINS`
- Cloudflare Worker vars：同名 `NEXT_PUBLIC_UMAMI_*` 变量，当前在 `wrangler.jsonc` 中维护公开值
...
需要同时填写 `NEXT_PUBLIC_UMAMI_WEBSITE_ID` 和 `NEXT_PUBLIC_UMAMI_SCRIPT_URL`。缺任意一项时 Umami 不会加载，避免误发到错误的统计服务。写完后需要重新部署 Cloudflare Worker。
...
这个白名单用于避免 `localhost`、预览域名和本地调试页面向自部署 Umami 上报，避免污染真实访问数据。
...
本地 `.env.local` 只影响本机。线上还需要把同一个值写到 GitHub Actions secrets / vars 和 Cloudflare Worker vars：

- GitHub repository secret：`NEXT_PUBLIC_GA_MEASUREMENT_ID`
- Cloudflare Worker var：`NEXT_PUBLIC_GA_MEASUREMENT_ID`
...
- Cloudflare / GitHub Actions 生产环境已配置 `NEXT_PUBLIC_GA_MEASUREMENT_ID`。
- 已重新部署。
