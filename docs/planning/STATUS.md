...
| --- | --- | --- | --- | --- | --- |
| 规划文档流转 | 已完成，待维护 | 三状态归档规则已建立；2026-07-11 完成首轮旧产品规划复盘 | 本轮将 8 份已完成、已被替代或明确关闭的旧规划移入 `done`；当前文件分布为待处理 30 个、进行中 8 个、已完成 95 个 | 后续新规划按状态流转；每次启动或完成规划时同步移动文档并补执行记录 | [规划流转归档](archive/README.md) |
...
| P4 基础设施和成本优化 | P1 本地实现完成，待生产确认 | 24 小时基线完成；DO Queue / prefetch 收敛已提交，rollup / dry-run retention migration 已本地验证 | 生产 WAF / Rate Limiting、日志采样、应用部署、migration 应用与删除数据均未执行；下一步只在明确确认后进入 Preview / production；见[基础设施容量、异常流量与成本治理规划](archive/in-progress/product/2026-07-14_priceai-infrastructure-capacity-traffic-and-cost-governance-plan.md) |
